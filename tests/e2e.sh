...
# --- test 1: index 0 (Tap to click) toggles on activate --------------------
b0="$(getb tap_to_click)"
open_panel || skip "panel would not open via IPC (bar widget idle?)"
activate                             # index 0 -> setToggle(tap_to_click)
...
if [ -z "$a0" ] || [ "$a0" = "$b0" ]; then
  ...
  [ -f "$CFG" ] && [ "$(jq -r '.touchpad.tapToClick' "$CFG")" != "null" ] \
...
open_panel
nav 2; activate                      # index 0 -> 2 -> setToggle(natural_scroll)
...
# --- test 3: scroll-speed segmented control (index 6, last nav stop) -------
rm -f "$CFG" "$LUA"
b3="$(getf scroll_factor)"
open_panel
nav 6; activate                      # 6 toggles then the scroll stop -> cycleScroll()
